...
import Blockly, { Input } from "blockly/core";
...
const procedureDefMutator = {
  ...
  //   .appendField(dropdownField, "DROPDOWNVALUE_"+argId)
  ...
  addVarInput_: function (name, argId) {
    ...
    console.log("count", this.argData_.length);

    // Append dropdownField with unique id
    const input = this.appendDummyInput(argId)
      .setAlign(Blockly.ALIGN_RIGHT)
      .appendField(createMinusField(argId));

    if (this.argData_.length >= 1) {
      // Show "AND" and subsequent fields only if there are already some fields
      input.appendField("AND");
    }

    input
      .appendField(dropdownField, "DROPDOWNVALUE_" + argId) // Update the field name
      .appendField("<=")
      .appendField(nameField, argId); // The name of the field is the arg id.
  },
...
const procedureDefHelper = function () {
  ...
  this.initProcedureBlock();

  this.getInput("TOP").insertFieldAt(0, createPlusField(), "PLUS");
};

/**
 * Initialize procedure block, hide the "NAME" input field.
 * @this {Blockly.Block}
 */
Blockly.Block.prototype.initProcedureBlock = function () {
  // Iterate through the inputList and hide the "NAME" field
  for (let i = 0; i < this.inputList.length; i++) {
    const input = this.inputList[i];
    if (input.fieldRow) {
      for (let j = 0; j < input.fieldRow.length; j++) {
        const field = input.fieldRow[j];
        if (field.name === "NAME" && field instanceof Blockly.FieldTextInput) {
